...
const decode = (s: string) => {
  const e = sharedDoc.createElement('textarea')
  e.innerHTML = s ?? ''
  return e.value
}
...
const tLink = (text: string, url: string): any => ({
  type: 'link',
  fields: { linkType: 'custom', newTab: !url.startsWith('/') && !url.includes('palisad'), url },
  children: [tText(text)],
  direction: 'ltr',
  format: '',
  indent: 0,
...
const tParagraph = (children: any[]): any => ({
  type: 'paragraph',
  children: children.length ? children : [tText('')],
  direction: 'ltr',
  format: '',
  indent: 0,
  textFormat: 0,
...
const tHeading = (text: string, tag: 'h1' | 'h2' | 'h3'): any => ({
  type: 'heading',
  tag,
  children: [tText(text)],
  direction: 'ltr',
...
function autolink(text: string, fmt: number): any[] {
  const re =
    /([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Za-z]{2,})|(https?:\/\/[^\s<>()]+)|(\+\d[\d\s().\-/]{7,}\d)|(pravila o kola[čc]i[ćc]ima|politik[aue] kola[čc]i[ćc]a)|(pravila o privatnosti|politik[aue] privatnosti)/giu
  const out: any[] = []
  let last = 0
...
function inlineNodes(el: Element): any[] {
  ...
  const walk = (node: ChildNode, fmt: number) => {
    if (node.nodeType === 3) {
      ...
      if (t.trim()) out.push(...autolink(decode(t).replace(/\s+/g, ' '), fmt))
      return
    }
    ...
    if (tag === 'a') {
      const href = fixUrl(e.getAttribute('href') || '#')
      const text = collapse(e.textContent ?? '')
      if (text) out.push(tLink(text, href))
      return
    }
    if (tag === 'br') return
    const nf =
...
function parseLegal(
  ...
  title: string,
  slug: string,
): { nodes: any[]; description: string; updated: string } {
  SELF_POLICY =
    slug === 'pravila-o-kolacicima' ? 'cookie' : slug === 'politika-privatnosti' ? 'privacy' : ''
  ...
  const captureMeta = (plain: string) => {
    const m = plain.match(
      /(?:poslednji put a[žz]urirano|datum stupanja na snagu)[^.\d]*([\d]{1,2}\.?\s*\p{L}+\s*\d{4}|\d{1,2}\.\s*\w+\s*\d{4})/iu,
    )
  ...
  const handle = (el: Element) => {
    ...
    if (tag === 'h1' || tag === 'h2') {
      ...
      if (/poslednji put|datum stupanja/i.test(plain)) {
        captureMeta(plain)
        return
      } // moves to hero
      const strong = el.querySelector('strong, b')
      const fullyBold = strong && collapse(strong.textContent ?? '') === plain
      if (fullyBold && plain.length < 90) {
        nodes.push(tHeading(plain.replace(/:$/, ''), 'h3'))
        return
      }
      ...
      const sectionLike = lis.some(
        (li) =>
          li.querySelector('p') ||
          (li.querySelector('strong, b') && collapse(li.textContent ?? '').length > 110),
      )
      if (sectionLike) {
        lis.forEach((li, idx) => {
          ...
          if (ps.length) {
            ps.forEach((p) => {
              if (collapse(p.textContent ?? '')) nodes.push(tParagraph(inlineNodes(p as Element)))
            })
          } else {
            ...
            if (rest.some((n) => (n.text ?? '').trim() || n.type === 'link'))
              nodes.push(tParagraph(rest))
          }
        })
      } else {
...
const contentBlock = (nodes: any[]) => ({
  blockType: 'content',
  columns: [{ size: 'full', richText: tRoot(nodes) }],
})
...
const run = async () => {
  ...
  for (const { slug, fallbackTitle } of PAGES) {
    const res = await fetch(`${SITE}/wp-json/wp/v2/pages?slug=${slug}&_fields=slug,title,content`, {
      headers: { 'User-Agent': UA },
    })
    const arr = await res.json()
    const src = Array.isArray(arr) ? arr[0] : null
    if (!src) {
      console.warn(`  ⚠ ${slug}: nije nađeno na izvoru`)
      continue
    }
    ...
    console.log(
      `  ▸ ${slug} — "${title}" (${nodes.length} blokova)${updated ? ` [${updated}]` : ''}`,
    )

    legalLinks.push({
      link: { type: 'custom', appearance: 'default', label: title, url: `/${slug}`, newTab: false },
    })
    ...
    if (DRY_RUN) {
      nodes
        ...
        .forEach((n) =>
          console.log(
            `     ${n.type}${n.tag ? `(${n.tag})` : ''}: ${(n.children?.[0]?.text || n.children?.[0]?.children?.[0]?.text || '').slice(0, 70)}`,
          ),
        )
      continue
    }
    ...
    const data: any = {
      ...
      meta: {
        ...
        description: description || `${title} — Palisada d.o.o.`,
      },
    }
