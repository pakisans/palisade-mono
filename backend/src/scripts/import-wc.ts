...
function parseCSV(content: string): WCRow[] {
  ...
  const headerMap: Record<string, keyof WCRow> = {
    ...
    'Ime 1 atributa': 'attr1Name',
    'Atribut 1 vrednost(i)': 'attr1Values',
    'Atribut 1 je vidljiv': 'attr1Visible',
    'Ime 2 atributa': 'attr2Name',
    'Atribut 2 vrednost(i)': 'attr2Values',
    'Atribut 2 je vidljiv': 'attr2Visible',
    'Ime 3 atributa': 'attr3Name',
    'Atribut 3 vrednost(i)': 'attr3Values',
    'Atribut 3 je vidljiv': 'attr3Visible',
    'Ime 4 atributa': 'attr4Name',
    'Atribut 4 vrednost(i)': 'attr4Values',
    'Atribut 4 je vidljiv': 'attr4Visible',
  }
...
function htmlToLexical(html: string): object {
  ...
  const normalized = normalizeEscapedLineBreaks(html)
    .replace(/<br\s*\/?>/gi, '\n')
    .replace(/<\/p>/gi, '\n')
    .replace(/<\/h[1-6]>/gi, '\n')
    .replace(/<\/li>/gi, '\n')
  ...
  for (const seg of segments) {
    ...
    if (h2) {
      const t = stripHtml(h2[1])
      if (t) children.push(makeHeading('h2', t))
      continue
    }
    if (h3) {
      const t = stripHtml(h3[1])
      if (t) children.push(makeHeading('h3', t))
      continue
    }
    if (h4) {
      const t = stripHtml(h4[1])
      if (t) children.push(makeHeading('h4', t))
      continue
    }
    if (/<h[1-6]/.test(seg)) {
...
async function main() {
  ...
  if (!csvPath) {
    console.error('Usage: pnpm import:wc <csv-path>')
    process.exit(1)
  }
  ...
  const parents = allRows
    .filter((r) => r.type === 'simple' || r.type === 'variable')
  ...
  if (LIMIT) {
    productsToImport = productsToImport.slice(0, LIMIT)
  ...
  const resolvedVarCount = variableParents.reduce(
    (sum, r) => sum + (variationsByParent.get(r.id)?.length ?? 0),
    0,
  )

  console.log(
    `📦 Parents: ${productsToImport.length} (${variableParents.length} variable, ${simpleParents.length} simple)`,
  )
  console.log(
    `🔀 Variations in CSV: ${variations.length} | Resolved to parents: ${resolvedVarCount} | Orphaned: ${variations.length - resolvedVarCount}`,
  )
  ...
  if (variations.length - resolvedVarCount > 0) {
    ...
    for (const o of orphanedRows) {
      console.warn(
        `  ⚠ Orphan variation ID:${o.id} "${o.name?.slice(0, 40)}" → parent "${o.parent}" not found`,
      )
    }
  }
  ...
  for (const path of sortedPaths) {
    ...
    try {
      const c = await payload.create({
        collection: 'categories',
        data: { title, slug, ...(parentId ? { parent: parentId } : {}) },
      })
  ...
  for (const attrName of allAttrNames) {
    const existing = await payload.find({
      ...
      where: { label: { equals: attrName } },
      limit: 1,
    })
    if (existing.docs.length > 0) {
      variantTypeMap.set(attrName, existing.docs[0].id as number)
      process.stdout.write('.')
      continue
    }
    try {
      const vt = await payload.create({
        collection: 'variantTypes',
        data: { label: attrName, name: slugify(attrName) },
      })
  ...
  for (let i = 0; i < productsToImport.length; i++) {
        ...
        for (const attr of attrs) {
          ...
          for (const valueStr of allValues) {
            ...
            const existing = await payload.find({
              ...
              where: {
                and: [{ label: { equals: valueStr } }, { variantType: { equals: typeId } }],
              },
              limit: 1,
            })
            if (existing.docs.length > 0) {
    ...
    if ((i + 1) % 5 === 0 || i === productsToImport.length - 1) {
      process.stdout.write(
        `\r  [${i + 1}/${productsToImport.length}] created:${created} skipped:${skipped} failed:${failed}  `,
      )
    }
  }
