...
export default async function SavetiList({ current = 1 }) {
  ...
  const totalPages = postsRes?.totalPages ?? Math.max(1, Math.ceil(total / PAGE_SIZE))
        ...
        <div className="container-site">
          <ScrollReveal className="mx-auto mb-10 max-w-3xl text-center md:mb-12">
            <div className="mb-4 inline-flex items-center gap-2 text-sm font-semibold text-brand">
              <span className="inline-flex h-8 w-8 items-center justify-center rounded-full bg-brand/10" aria-hidden="true">
            ...
            <h1 id="advice-heading" className="text-3xl font-extrabold tracking-tight text-gray-950 md:text-5xl">
          ...
          {posts.length > 0 ? (
            <>
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 md:gap-8 lg:grid-cols-3">
                {posts.map((post, i) => (
                  <ScrollReveal key={post.id} delay={Math.min(i % 3, 2) * 80} className="h-full">
                    <AdviceCardClassic post={post} priority={i < 3} />
                  </ScrollReveal>
                ))}
              </div>
              <Pagination basePath="/saveti" current={current} total={totalPages} />
            </>
          ) : (
            <div className="rounded-2xl border border-dashed border-gray-200 bg-gray-50 p-8 text-center">
              <p className="text-sm font-semibold text-gray-600">Saveti trenutno nisu dostupni.</p>
            </div>
          )}
        </div>
      </section>
