      ...
      <Link href={href} tabIndex={-1} aria-hidden="true" className="block relative overflow-hidden aspect-[4/3] bg-gray-50">
        {imgUrl ? (
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
            <span className="text-5xl font-extrabold text-gray-200 select-none">{product.title.charAt(0)}</span>
          </div>
        )}
          ...
          <div className="flex items-baseline gap-2">
            {hasPrice ? (
              <>
                ...
                {hasSale && (
                  <span className="text-[12px] text-gray-400 line-through leading-none">{formatPrice(price)}</span>
                )}
              </>
            ) : (
              <span className="text-[13px] font-semibold text-gray-400 italic">Cena na upit</span>
            )}
          </div>
