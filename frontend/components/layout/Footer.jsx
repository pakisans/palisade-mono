...
function groupCategories(categories) {
  ...
  return parents
    .map((p) => ({
      ...p,
      children: docs.filter((c) => parentIdOf(c) === p.id),
    }))
    .sort((a, b) => {
    ...
    <div>
      ...
      >
        {logoUrl ? (
          ...
          <>
            <span className="flex items-center justify-center w-9 h-9 rounded-lg bg-brand text-white font-bold text-lg">
              {name.charAt(0)}
            </span>
      ...
      {section.tagline && (
        <p className="text-sm font-semibold text-slate-300 mb-2 italic">
          "{section.tagline}"
        </p>
      )}
      {section.description && (
...
function FooterTextBlock({ section }) {
  ...
  return (
    <p className="text-sm text-slate-400 leading-relaxed col-span-full">
      {section.text}
    </p>
  );
}
...
function FooterBlock({ section, siteLogoUrl, siteName }) {
  switch (section.blockType) {
    ...
    case 'footerColumn':
      return <FooterColumnBlock section={section} />;
    case 'footerContact':
      return <FooterContactBlock section={section} />;
    case 'footerSocial':
      return <FooterSocialBlock section={section} />;
    case 'footerText':
      return <FooterTextBlock section={section} />;
    case 'footerNewsletter':
      return <FooterNewsletterBlock section={section} />;
    default:
      return null;
  }
}
